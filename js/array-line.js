...
class Editor {
  ...
  setupUI() {
    ...
    document.getElementById('findIntersections').addEventListener('click', () => {
      ...
      // Временно показываем точки пересечения на холсте
      this.showIntersections = true;
      this.intersectionPoints = intersections.flatMap(i => i.points);

      // Перерисовываем с точками пересечения
      this.redraw();

      // // Через 10 секунд скрываем точки пересечения
      // setTimeout(() => {
      //   this.showIntersections = false;
      //   this.intersectionPoints = [];
      //   this.redraw();
  ...
  findIntersections() {
    ...
    // Проверяем каждую линию с каждым объектом
    ...
    console.log('=== ПЕРЕСЕЧЕНИЯ ЛИНИЙ И ОБЪЕКТОВ ===');
