...
class Editor {
	...
	drawIntersectionEndpoints(ctx = this.ctx) {
		if (!this.selectedElement || !this.selectedElement.intersections) return;

		this.selectedElement.intersections.forEach(intersection => {
	...
	redraw() {
		...
		// Добавляем отрисовку маркеров концов линий для выбранной точки пересечения
