...
class CanvasObject {
	constructor(type, x, y, width, height, color = '#3498db', label = 'Объект') {
		...
		this.type = type;
		// Теперь x и y - это центр объекта
		this.x = x - width / 2;
		this.y = y - height / 2;
		this.width = width;
		this.height = height;
	...
	// Получить вершины объекта с учетом поворота
	getVertices() {
		...
		// Применяем поворот и смещаем к центру
	...
	isPointInside(x, y) {
		// Проверка с учетом поворота
		const center = this.center;
		const angle = -this.rotation * Math.PI / 180;

		// Переводим точку в систему координат объекта
		...
		// Проверяем, находится ли точка в ненагруженном прямоугольнике
	...
	setSize(width, height) {
		...
		this.width = width;
		this.height = height;
		// Сохраняем центр при изменении размеров
...
class Editor {
	...
	getLineEndpointFromIntersection(intersection, specificLine = null) {
		if (intersection.type === 'line-line') {
			if (specificLine === 'line1') {
				return intersection.line1Endpoint;
			} else if (specificLine === 'line2') {
				return intersection.line2Endpoint;
			} else {
				// Для общего случая - это используется при фильтрации
				// Вам нужно определить, какую линию вы хотите проверить
				// В данном контексте мы проверяем обе линии
				return null;
			}
		} else if (intersection.type === 'line-object') {
			return intersection.lineEndpoint;
		}
		return null;
	}
	...
	createObjectIntersectionPoints(line, intersections) {
		...
		// Обновляем значения tr после добавления точек пересечения
	...
	deleteAtPosition(pos) {
		...
		if (lineToDelete) {
			...
			// Удаляем сохраненное значение tr
			this.originalTrValues.delete(lineToDelete.id);
	...
	findIntersections() {
		...
		this.updateLineTrackProperties(); // Это теперь включает обновление tr
	...
	updatePassabilityForAllLines() {
		...
		// Обновляем значения tr после обновления passability
		this.updateAllTrValues();
	}
	...
	redraw() {
		...
		if (this.needsRedraw) {
			...
			this.lines.forEach(line => {
				...
				this.drawLineProperties(line, bufferCtx); // Теперь включает tr
			});
	...
	showIntersectionInfo(point) {
		...
		if (point.intersections.length > 0) {
			...
			// Добавляем информацию о примененном условии для tr
			message += `\nПримененное условие для свойства tr:\n`;
			if (point.intersections.length === 2) {
				const hasStart = point.intersections.some(i => i.lineEndpoint === 'start');
				const hasEnd = point.intersections.some(i => i.lineEndpoint === 'end');
				if (hasStart && hasEnd) {
					message += `• 2 линии (одна началом, другая концом) - tr осталось без изменений\n`;
				} else {
					message += `• 2 линии (не подходят под условие) - tr не менялось\n`;
				}
			} else if (point.intersections.length > 2) {
				const startLinesCount = point.intersections.filter(i => i.lineEndpoint === 'start').length;
				message += `• ${point.intersections.length} линий (${startLinesCount} подходят началом) - tr поделено для линий, подходящих началом\n`;
			}
		} else {
	...
	formatLineLineIntersection(intersection, point) {
		...
		if (line1 && line1.tr !== undefined) {
			message += `    Свойство tr линии 1: ${line1.tr.toFixed(2)}\n`;
		}
		if (line2 && line2.tr !== undefined) {
			message += `    Свойство tr линии 2: ${line2.tr.toFixed(2)}\n`;
		}
	...
	formatLineObjectIntersection(intersection, point) {
		...
		if (line && line.tr !== undefined) {
			message += `    Свойство tr линии: ${line.tr.toFixed(2)}\n`;
		}
	...
	logAllLines() {
		...
		console.log('Значения tr:', Array.from(this.originalTrValues.entries()));
