  ...
  static createObject(type, x, y) {
    const objectConfigs = {
      ...
      'boom': {
        ...
        label: 'Массовый взрывные работы'
      },
      'boom2': {
        width: 40,
        height: 40,
...
class Editor {
  ...
  handleMouseDown(e) {
    ...
    if (this.mode === 'draw' && this.currentObjectType) {
      // ИСПРАВЛЕНИЕ: Проверяем, не находится ли точка добавления на существующей линии
      const lineAtPoint = this.findLineAtPoint(snappedPos);
      if (lineAtPoint && this.isPointOnLine(snappedPos, lineAtPoint, 10)) {
        // Разделяем линию перед добавлением объекта
        this.splitLine(lineAtPoint, snappedPos);
      }
  ...
  // Рисование линий с возможностью разделения существующих линий
  startDrawing(pos) {
    this.isDrawing = true;

    // Проверяем, начинаем ли мы рисовать с существующей линии
    if (!this.currentObjectType) {
      const lineAtPoint = this.findLineAtPoint(pos);
      if (lineAtPoint && this.isPointOnLine(pos, lineAtPoint, 10)) {
        // Запоминаем линию для разделения
  ...
  finishDrawing() {
    if (this.isDrawing && this.tempLine) {
      ...
      if (length > 5) {
        // Если мы начали рисовать с существующей линии, разделяем ее
  ...
  // Добавление объектов - ИСПРАВЛЕННЫЙ МЕТОД
  addObject(type, x, y) {
