...
const ImageManager = (() => {
	...
	return {
		loadImage,
		preloadImages,
		getCachedImage: (url) => imageCache.get(url),
...
const ObjectFactory = (() => {
	...
	const createImageObject = async (type, x, y, width, height, imageUrl, label = 'Объект') => {
		const obj = createCanvasObject(type, x, y, width, height, '#3498db', label);
		obj.isImageObject = true;
		obj.properties = { imageUrl };

		try {
			// Пытаемся загрузить изображение из кэша или загрузить его
			const img = await ImageManager.loadImage(imageUrl);
			obj.image = img;
			obj.imageLoaded = true;
		} catch (error) {
			console.error('Не удалось загрузить изображение:', imageUrl, error);
			obj.image = null;
	...
	return {
		createObject: async (type, x, y) => {
			const config = objectConfigs[type];
			if (config) {
				return await createImageObject(type, x, y, config.width, config.height, config.image, config.label);
			}
		...
		preloadAllImages: async () => {
			const urls = Object.values(objectConfigs).map(config => config.image);
			return await ImageManager.preloadImages(urls);
		}
	};
})();
...
const DrawingFunctions = {
	...
	drawObject: (ctx, obj) => {
		if (obj.isImageObject && obj.image && obj.imageLoaded) {
			// Рисуем изображение
			ctx.save();
			...
			ctx.restore();
		} else if (obj.isImageObject && !obj.imageLoaded) {
			// Для объектов с изображениями, которые еще не загрузились, рисуем красивую заглушку
			ctx.save();
			...
			ctx.textAlign = 'center';
			ctx.textBaseline = 'middle';
			ctx.fillText('Загрузка...', obj.x + obj.width / 2, obj.y + obj.height / 2);

			// Иконка загрузки (простой круг)
			ctx.beginPath();
			ctx.arc(obj.x + obj.width / 2, obj.y + obj.height / 2 - 10, 8, 0, Math.PI * 2);
			...
			// Для обычных объектов рисуем цветной прямоугольник
		...
		ctx.textAlign = 'center';
		ctx.textBaseline = 'top';
		ctx.fillText(obj.label, obj.x + obj.width / 2, obj.y + obj.height + 5);
	},
...
const createEditor = (canvasId) => {
	...
	// Предзагрузка изображений при инициализации
	const preloadImages = async () => {
		try {
			await ObjectFactory.preloadAllImages();
	...
	const handleMouseDown = async (e) => {
		...
		if (state.mode === 'draw' && state.currentObjectType) {
			await addObject(state.currentObjectType, snappedPos.x, snappedPos.y);
			return;
		}
	...
	const addObject = async (type, x, y) => {
		try {
			const obj = await ObjectFactory.createObject(type, x, y);
			const newObjects = [...state.objects, obj];
			updateObjects(newObjects);

			// ⬇️ КРИТИЧНОЕ ИЗМЕНЕНИЕ: Пересчёт пересечений после добавления объекта
			const splitResult = IntersectionFunctions.splitAllIntersectingLines(state.lines);
			if (splitResult.changed) {
				console.log('Линии были разбиты после добавления объекта');
				updateLines(splitResult.lines);
			}
			...
			alert('Не удалось создать объект. Проверьте консоль для подробностей.');
		}
	};
...
document.addEventListener('DOMContentLoaded', async () => {
	...
	// Предупреждение пользователю, что изображения загружаются
	console.log('Редактор инициализирован. Изображения предзагружаются...');
