...
class IntersectionPoint {
  ...
  // Проверка близости точек (порог 5 пикселей)
...
class Editor {
  ...
  mergeCloseIntersectionPoints(allPoints) {
    ...
    for (const point of allPoints) {
      ...
      for (const mergedPoint of mergedPoints) {
        if (mergedPoint.isNear(point, 5)) { // Увеличенный порог до 5 пикселей
  ...
  // Метод: Удаление дубликатов точек
  removeDuplicateIntersectionPoints() {
    ...
    for (const point of this.intersectionPoints) {
      ...
      for (const uniquePoint of uniquePoints) {
        if (point.isNear(uniquePoint, 5)) { // Используем увеличенный радиус
      ...
      if (!isDuplicate && !usedIds.has(point.id)) {
  ...
  findIntersections() {
    ...
    // 4. Удаление дубликатов (дополнительная проверка)
    ...
    console.log(`Слияние близких точек (радиус 5px): ${allIntersectionPoints.length - this.intersectionPoints.length} точек объединено`);
  ...
  isPointOnLineSegment(point, lineStart, lineEnd, tolerance = 2) {
  ...
  getLineObjectIntersections(line, obj) {
    ...
    for (let i = 0; i < bounds.length; i++) {
      ...
      if (intersection && this.isPointOnLineSegment(intersection, line.start, line.end, 2)) {
    ...
    for (const point of intersectionPoints) {
      let isUnique = true;
      for (const uniquePoint of uniquePoints) {
        const dx = Math.abs(point.x - uniquePoint.x);
        const dy = Math.abs(point.y - uniquePoint.y);
        if (dx < 5 && dy < 5) {
          isUnique = false;
          break;
        }
