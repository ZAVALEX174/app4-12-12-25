...
class Editor {
    ...
    const trData = this.lines.map(line => {
      ...
      return {
        ...
        properties: {
          ...
          roughnessCoef: line.roughnessCoef,
          crossSectionalArea: line.crossSectionalArea
        }
      };
    });
  ...
  startDrawing(pos) {
    ...
    this.tempLine = {
      ...
      roughnessCoef: this.roughnessCoef,
  ...
  showLinePropertiesPanel(line) {
    ...
    const lengthLineInput = document.getElementById('lineLengthLine');
    const roughnessCoefInput = document.getElementById('lineRoughnessCoef');
    const crossSectionalAreaInput = document.getElementById('lineCrossSectionalArea');
    const trInput = document.getElementById('lineTr');

    if (!panel || !lengthLineInput || !roughnessCoefInput || !crossSectionalAreaInput) return;

    lengthLineInput.value = line.lengthLine || '';
    roughnessCoefInput.value = line.roughnessCoef || '';
    crossSectionalAreaInput.value = line.crossSectionalArea || '';
  ...
  applyLineProperties() {
    ...
    if (!lengthLineInput || !roughnessCoefInput || !crossSectionalAreaInput) return;

    this.selectedElement.lengthLine = lengthLineInput.value ? parseFloat(lengthLineInput.value) : null;
  ...
  showAllLinesProperties() {
    ...
    linesProperties.forEach((line, index) => {
      ...
      message += `  Длина линии (lengthLine): ${line.lengthLine || 'не задана'}\n`;
      message += `  Коэффициент шероховатости (roughnessCoef): ${line.roughnessCoef || 'не задана'}\n`;
      message += `  Площадь поперечного сечения (crossSectionalArea): ${line.crossSectionalArea || 'не задана'}\n`;
