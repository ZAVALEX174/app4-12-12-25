...
class Editor {
	...
	setupUI() {
		...
		// Новые кнопки для работы со свойствами passability
		const updatePassabilityBtn = document.getElementById('updatePassability');
		if (updatePassabilityBtn) {
			updatePassabilityBtn.addEventListener('click', () => {
				this.updatePassabilityForAllLines();
				this.needsRedraw = true;
				this.redraw();
				alert('Свойства passability обновлены для всех линий');
			});
		}

		const showPassabilityInfoBtn = document.getElementById('showPassabilityInfo');
		if (showPassabilityInfoBtn) {
			showPassabilityInfoBtn.addEventListener('click', () => {
				if (this.selectedElement && this.selectedElement.start && this.selectedElement.end) {
	...
	getLineEndpointAtIntersection(line, intersectionPoint, intersectionId, tolerance = 5) {
		...
		if (distToStart < tolerance && distToEnd < tolerance) {
			...
			if (endpoint === 'start') {
				...
				// Устанавливаем passability = 5 для начала линии
				if (intersectionId) {
					line.passability[intersectionId] = 5;
				}
			} else {
				...
				// Устанавливаем passability = 10 для конца линии
				if (intersectionId) {
					line.passability[intersectionId] = 10;
				}
			}
			return endpoint;
			...
			// Устанавливаем passability = 0 для середины линии
			if (intersectionId) {
				line.passability[intersectionId] = 0;
			}
			return 'middle';
		}
	}
			...
			for (let i = 0; i < uniquePoints.length - 1; i++) {
				...
				if (segmentLength > 5) {
					const newLine = {
						...
						endtrack: [],
						passability: {} // Инициализируем пустым
					};
	...
	updateLineTrackProperties() {
		...
		this.lines.forEach(line => {
			line.track = [];
			line.endtrack = [];
			line.passability = {}; // Очищаем passability
		});
				...
				if (intersection.type === 'line-line') {
					...
					if (line1) {
						if (intersection.line1Endpoint === 'start') {
							...
							// Устанавливаем passability = 5 для начала линии
							line1.passability[intersectionPoint.id] = 5;
							...
							// Устанавливаем passability = 10 для конца линии
							line1.passability[intersectionPoint.id] = 10;
						} else {
							// Для середины линии устанавливаем passability = 0
							line1.passability[intersectionPoint.id] = 0;
						}
					}
					...
					if (line2) {
						if (intersection.line2Endpoint === 'start') {
							...
							// Устанавливаем passability = 5 для начала линии
							line2.passability[intersectionPoint.id] = 5;
							...
							// Устанавливаем passability = 10 для конца линии
							line2.passability[intersectionPoint.id] = 10;
						} else {
							// Для середины линии устанавливаем passability = 0
							line2.passability[intersectionPoint.id] = 0;
						}
					}
				}
			});
		...
		// Для линий без пересечений устанавливаем passability по умолчанию
		this.lines.forEach(line => {
		...
		this.lines.forEach((line, index) => {
			...
			console.log(`  passability:`, line.passability);
		});
	}

	// Метод: Установка свойства passability для всех линий
	updatePassabilityForAllLines() {
		console.log('=== Обновление свойств passability для всех линий ===');

		this.lines.forEach(line => {
			// Инициализируем объект passability, если его нет
			if (!line.passability) {
				line.passability = {};
			} else {
			...
			this.intersectionPoints.forEach(point => {
				// Ищем пересечение с текущей линией
				...
				if (intersection) {
					...
					// Устанавливаем значение passability в зависимости от конца линии
					if (lineEndpoint === 'start') {
						// Линия подходит началом - значение 5
						line.passability[point.id] = 5;
						console.log(`Линия ${line.id}: в точке ${point.id} подходит началом → passability = 5`);
					} else if (lineEndpoint === 'end') {
						// Линия подходит концом - значение 10
						line.passability[point.id] = 10;
						console.log(`Линия ${line.id}: в точке ${point.id} подходит концом → passability = 10`);
					} else if (lineEndpoint === 'middle') {
						// Точка в середине линии - значение 0 (или другое по умолчанию)
						line.passability[point.id] = 0;
						console.log(`Линия ${line.id}: точка ${point.id} в середине линии → passability = 0`);
					}
				}
			});

			// Если у линии нет пересечений, устанавливаем passability = 0
			if (Object.keys(line.passability).length === 0) {
				line.passability.default = 0;
				console.log(`Линия ${line.id}: нет пересечений → passability = 0`);
			}
		});

		console.log('=== Свойства passability обновлены ===');
	}

	// Метод: Установка свойства passability для одной линии
	...
	// Метод: Получение значения passability для линии в конкретной точке
	...
	// Метод: Получение суммарного passability для линии
	getLineTotalPassability(line) {
		...
		let total = 0;
		Object.values(line.passability).forEach(value => {
			total += value;
		});
	...
	// Метод: Визуализация свойств passability на холсте
	drawPassabilityProperties(ctx = this.ctx) {
		if (!this.showTrackProperties) return;

		this.lines.forEach(line => {
			if (line.passability) {
				// Рисуем значения passability в точках пересечения
				Object.entries(line.passability).forEach(([pointId, value]) => {
					// Пропускаем значение по умолчанию
					...
					if (point) {
						...
						let color;
						if (value === 5) {
							color = 'rgba(0, 255, 0, 0.7)'; // Зеленый для passability 5
						} else if (value === 10) {
							color = 'rgba(255, 0, 0, 0.7)'; // Красный для passability 10
						} else {
							color = 'rgba(255, 255, 0, 0.7)'; // Желтый для других значений
						}
						...
						// Текст со значением passability
						ctx.fillStyle = 'white';
	...
	// Метод: Показать информацию о passability линии
			...
			Object.entries(line.passability).forEach(([pointId, value]) => {
				if (pointId === 'default') {
					...
					if (point) {
						let endpointInfo = '';
						if (value === 5) {
							endpointInfo = '(подходит началом)';
						} else if (value === 10) {
							endpointInfo = '(подходит концом)';
						}
			...
			Object.entries(line.passability).forEach(([pointId, value]) => {
				if (pointId === 'default') {
					...
					if (point) {
						let endpointInfo = '';
						if (value === 5) {
							endpointInfo = '(начало линии)';
						} else if (value === 10) {
							endpointInfo = '(конец линии)';
						}
		...
		if (point.intersections.length > 0) {
			...
			point.intersections.forEach((intersection, index) => {
				...
				if (intersection.type === 'line-object') {
					...
					// Добавим информацию о passability
					const line1 = this.lines.find(l => l.id === intersection.line1Id);
					const line2 = this.lines.find(l => l.id === intersection.line2Id);
